...
export default function access(initialState: { currentUser?: API.CurrentUser } | undefined) {
  ...
  return {
    ...
    dealer_insert: (currentUser?.accessList.indexOf('dealer_insert') ?? -1) >= 0,

    // eslint-disable-next-line @typescript-eslint/no-shadow
    checkAccess: (_access: string) => {
      console.log(currentUser?.access, 'currentUser?.access');
      if (
        process.env.PLATFORM === 'MERCHANT' &&
        localStorage.getItem('Platform') + '_admin' === currentUser?.name
      ) {
        return true;
      }
      ...
      return (currentUser?.accessList.indexOf(_access) ?? -1) >= 0;
    },
  };
}
