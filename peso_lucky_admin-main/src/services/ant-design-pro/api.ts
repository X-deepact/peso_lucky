...
export async function optionDictDataSelect(
  params: {
    // query
    /** 字典类型 */
    dictType?: string;
  ...
  const res = await request<API.DictDataList>('/api/v1/dict-data/option-select', {
    method: 'GET',
    params: {
      ...params,
    },
  ...
  let data =
    res.data?.map((item) => {
      // console.log(
      //   formatMessage({
      //     id: `dict.${params.dictType}.${item.value}`,
      //     defaultMessage: item.label,
      //   }),
      //   `dict.${params.dictType}.${item.value}`,
      // );
      return {
        ...item,
        label:
  ...
  if (hasAll) {
    data = (data ?? []).concat({ value: '', label: formatMessage({ id: 'dict.ALL' }) });
  }
  return data;
}
